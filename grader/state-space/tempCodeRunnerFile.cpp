...
int main() {
    ...
    map<vector<vector<ll> >, ll > tableMove; // map that store type of table and its move count from original
    priority_queue<tmpPair, vector<tmpPair>, greater<tmpPair> > pq;
    ll costOfMoving = checkCost(table);
    pq.push(make_pair(costOfMoving, table));
    tableMove[table] = 0;
